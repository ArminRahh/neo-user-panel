document.addEventListener('DOMContentLoaded', function () {
    ...
    const menuItems = document.querySelectorAll('.neo-menu-item:not(.neo-logout)');
            ...
            fetch(neo_ajax_obj.ajax_url, {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        contentArea.innerHTML = '<p style="color:red;">خطایی رخ داد. لطفا دوباره تلاش کنید.</p>';
                    }
                })
                .catch(error => {
                    console.error('Error:', error);
                    contentArea.innerHTML = '<p style="color:red;">خطا در ارتباط با سرور.</p>';
                })
                .finally(() => {
    ...
    contentArea.addEventListener('click', function (e) {
        ...
        const modal = document.getElementById('neo-order-modal');
        if (modal) {
            if (e.target.classList.contains('neo-modal-close') || e.target === modal) {
                modal.style.display = 'none';
            }
        }
    });

    // --- منطق فرم کیف پول (هماهنگ‌سازی مبالغ و ارسال AJAX) ---
    ...
    contentArea.addEventListener('click', function (e) {
        ...
        if (e.target && e.target.id === 'neo-hide-custom-amount') {
            ...
            document.getElementById('neo-show-custom-amount').style.display = 'inline';
        }
    });
