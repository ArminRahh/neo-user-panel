   ...
   <div class="neo-wrapper">
      ...
      <aside class="neo-sidebar">
         ...
         <ul class="neo-menu">
            ...
            ?>
            <?php if ($is_admin || $vendor_status === 'approved' || $vendor_status === 'active') :  ?>
               <!-- منوی فروشندگان (برای کاربران تایید شده) -->
               <li class="neo-menu-item neo-has-submenu" data-tab="vendor-dashboard">
                  <i></i> پنل فروشندگان
               </li>
               <li class="neo-menu-item neo-has-submenu" data-tab="vendor-add-product">
...
function neo_ajax_handler()
{
   ...
   switch ($tab) {
      ...
      case 'vendor-add-product':
         ob_start();
         if (file_exists(plugin_dir_path(__FILE__) . 'includes/vendor/views/vendor-add-product.php')) {
            include plugin_dir_path(__FILE__) . 'includes/vendor/views/vendor-add-product.php';
         } else {
            echo '<p>فایل افزودن محصول یافت نشد.</p>';
         }
         $response_html = ob_get_clean();
         break;
